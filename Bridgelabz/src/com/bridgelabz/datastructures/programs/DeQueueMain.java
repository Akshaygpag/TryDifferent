...
public class DeQueueMain {
	...
	try {
	    ...
	    deQueu = new DeQueImpl();// CREATED AN EMPTY DEQUEUE
     ...
     * 
     */
    private static boolean checkForpalinDrome(Character[] userStringToCharArray, DeQue deQueu) {
	...
	while (low <= iterateTill) {
	     ...
	     if ((deQueu.removeRear().getInfo().compareTo(deQueu.removeFront().getInfo()))
	     != 0) {
